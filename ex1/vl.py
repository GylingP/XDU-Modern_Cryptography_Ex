...
def find_possible_keys(subarr, visible_chars):
    possible_keys = list(range(256))
    result_keys = list(possible_keys)

    for i in possible_keys:
        for s in subarr:
            decrypted_char = chr(s ^ i)
            if decrypted_char not in visible_chars:
                result_keys.remove(i)
                break

    return result_keys

def get_key_length_and_values(ciphertext):
    visible_chars = string.ascii_letters + string.digits + ',' + '.' + ' '
    ci_array = [int(ciphertext[x:x+2], 16) for x in range(0, len(ciphertext), 2)]
    vigenere_keys = []

    for index in range(7):
        subarr = ci_array[index::7]
        vigenere_keys.append(find_possible_keys(subarr, visible_chars))

    print("Possible Vigenere Keys:", bytes(k[0] for k in vigenere_keys))

    plaintext = ''
    for i in range(len(ci_array)):
        plaintext += chr(ci_array[i] ^ vigenere_keys[i % 7][0])
...
if __name__ == "__main__":
    ...
    get_key_length_and_values(ciphertext)
